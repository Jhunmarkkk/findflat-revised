...
<div class="container p-6 bg-white">
    <div class="container pt-4 pb-4 mx-5" >
        <div class="flex items-center">
            ...
            <h3 class="text-xl font-semibold">
                ...
                <div>
                    ...
                    <p style="font-size: 0.875rem; font-weight: normal;">{{ $account->email }}</p>
                    <p style="font-size: 0.875rem; font-weight: normal;">{{ $account->contact }}</p>
                </div>
        ...
        <div id="editProfileContainer" class="hidden absolute bg-white rounded-lg shadow-md p-10 w-104 mt-20 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
           ...
           {{--<form id="updateForm" action="{{ route('user') }}" method="post" enctype="multipart/form-data"> --}}a 

            @csrf
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Name:</label>
                    <input type="text" id="name" name="name" value="{{ auth()->user()->fname }} {{ auth()->user()->lname }}" placeholder="Enter your name" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>

                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700">Email:</label>
                    <input type="email" id="email" name="email" value="{{ auth()->user()->email }}" placeholder="Enter your email" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>

                <div>
                    <label for="phone" class="block text-sm font-medium text-gray-700">Phone:</label>
                    <input type="text" id="phone" name="phone" value="{{ auth()->user()->contact }}" placeholder="Enter your phone number" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
                ...
                <button type="submit" class="bg-primary text-red px-4 py-2 rounded-md hover:bg-primary-dark transition duration-300">Save Changes</button>
            </form>
        </div>
</div>

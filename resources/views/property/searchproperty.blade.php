...
@section('content')
    ...
    <div class="container p-6 bg-white">
        ...
        {{-- Search Area --}}
        <form action="{{ route('searchproperty') }}" method="GET" class="w-full max-w-xl relative flex mb-6">
            <span class="absolute left-4 top-3 text-lg text-gray-400">
                <i class="fas fa-search"></i>
            </span>
            <input type="text" name="search" class="w-full border border-primary border-r-0 pl-12 py-3 pr-3 rounded-l-md focus:outline-none" placeholder="Search by title...">
            <button type="submit" class="bg-primary border border-primary text-white px-8 rounded-r-md hover:bg-transparent hover:text-primary transition">Search</button>
        </form>
                ...
                <div class="grid grid-cols-2 gap-6 pt-4 pb-16 items-start">
                    @foreach ($properties->unique('id') as $property)
                        <div class="bg-white shadow rounded overflow-hidden group  hover:shadow-2xl hover:scale-105 transition">
                            <div class="relative">
                                <img src="{{ asset('/storage/images/' . optional($property->image)->image_path) }}" alt="Property Image" class="w-full h-56 mx-auto block">
